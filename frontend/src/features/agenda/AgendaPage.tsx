...
const defaultAppointmentValues: AppointmentValues = { patientId: "", date: dateInputValue(addDays(new Date(), 1)), time: "09:00", professionalName: "Dra. Helena Costa", type: "ONLINE" };
...
export function AgendaPage() {
  ...
  const createAppointment = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["appointments"] }); queryClient.invalidateQueries({ queryKey: ["dashboard"] }); setOpen(false); toast("Consulta simulada criada como pendente de confirmação."); },
  ...
  const reschedule = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["appointments"] }); queryClient.invalidateQueries({ queryKey: ["dashboard"] }); setRescheduleOpen(false); setSelectedAppointment(null); toast("Reagendamento simulado salvo como pendente de confirmação."); },
  ...
  const createBlock = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["calendar-blocks"] }); setBlockOpen(false); toast("Bloqueio de horário simulado criado."); },
  ...
  function onBlock(values: CalendarBlockValues) { createBlock.mutate({ startAt: toDemoIso(values.date, values.time), durationMinutes: values.durationMinutes, professionalName: "Dra. Helena Costa", reason: values.reason }); }
  ...
  return <div>
    <PageHeader eyebrow="Clínica / Agenda" title="Agenda" description="Organize a operação dos profissionais sem perder o contexto do atendimento." actions={<>
      ...
      <Button onClick={() => setOpen(true)}><Plus size={17} /> Simular consulta</Button>
    </>} />
