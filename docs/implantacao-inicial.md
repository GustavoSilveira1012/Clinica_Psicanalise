...
- O perfil `production` mantém `CLINICAL_DATA_ENABLED=false` e `CLINICAL_DATA_RELEASE_APPROVED=false`. O backend só libera dados clínicos quando ambos forem explicitamente `true`; o frontend tem as mesmas duas travas `VITE_*`. Não habilitar nenhuma delas enquanto base isolada, isolamento, storage, backup/restore e demais controles não tiverem sido aprovados. Nunca apontar o ambiente gratuito para a base existente.
